...
const allowedOrigins = [
  ...
  process.env.STAGING_ALLOWED_ORIGIN || 'https://newera-staging.placeholder.com'
];
...
app.post('/api/lead-submit', async (req, res) => {
  ...
  if (tags.includes('newera_solar_lead')) {
    ghlWebhookUrl = process.env.GHL_SOLAR_WEBHOOK_URL;
  } else if (tags.includes('newera_roofing_lead')) {
    ghlWebhookUrl = process.env.GHL_ROOFING_WEBHOOK_URL;
  } else if (tags.includes('newera_water_lead')) {
    ghlWebhookUrl = process.env.GHL_WATER_WEBHOOK_URL;
  }
