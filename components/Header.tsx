      ...
      <header className="w-full bg-white border-b border-[#E6EDF2] px-4 md:px-8 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="font-poppins font-bold text-lg leading-tight uppercase tracking-tight">New Era</span>
        ...
        <nav className="hidden lg:flex items-center gap-5 xl:gap-6 text-sm font-medium text-[#5F6F75]">
          {navLinks.slice(0, 6).map((link, idx) => (
            <a 
              key={idx} 
              href={link.href} 
              className={link.active ? "text-[#FF8A3D] border-b-2 border-[#FF8A3D] pb-1" : "hover:text-[#123B5D] pb-1"}
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="lg:hidden fixed top-[73px] left-0 right-0 bg-white border-b border-[#E6EDF2] z-40 shadow-lg p-4 flex flex-col gap-2 h-[calc(100vh-73px)] overflow-y-auto">
          {navLinks.map((link, idx) => (
            <a 
              key={idx}
              href={link.href}
              className={`text-lg font-poppins font-medium py-3 px-4 rounded-xl ${link.active ? 'bg-[#F5F7FA] text-[#FF8A3D]' : 'text-[#123B5D] hover:bg-[#F5F7FA]'}`}
