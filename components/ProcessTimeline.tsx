  ...
  return (
    <section className="mt-8 mb-4 py-12 px-6">
      <div className="text-center mb-16">
        <span className="text-[10px] font-bold uppercase text-[#5EC8E5] tracking-widest mb-1 block">Simple & Transparent</span>
        <h2 className="font-poppins font-bold text-2xl md:text-3xl text-[#123B5D]">Our Process</h2>
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="hidden md:block absolute top-[44px] left-[10%] right-[10%] h-[2px] bg-[#E6EDF2] z-0"></div>
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-4 relative z-10">
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-row md:flex-col items-center md:items-start text-left md:text-center flex-1 gap-6 md:gap-4 relative group">
              ...
              {idx !== steps.length - 1 && (
                <div className="md:hidden absolute top-[70px] bottom-[-20px] left-[44px] w-[2px] bg-[#E6EDF2] z-0"></div>
              )}
              
              {/* Number / Icon Circle */}
              <div className="w-[88px] h-[88px] shrink-0 bg-white border-4 border-[#F5F7FA] rounded-full flex flex-col items-center justify-center shadow-sm group-hover:border-[#FFE1C7] transition-colors relative z-10 md:mx-auto">
                <span className="text-xl mb-1">{step.icon}</span>
                <span className="text-[10px] font-bold text-[#FF8A3D]">{step.num}</span>
              </div>
              
              {/* Content */}
              <div className="flex flex-col flex-1 pt-2 md:pt-0">
                <h4 className="font-poppins font-bold text-lg text-[#123B5D] mb-2">{step.title}</h4>
