  ...
  return (
    <div>
      ...
      {visibleReviews.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {visibleReviews.map((review, idx) => (
            <div key={idx} className="bg-white border border-[#E2E8F0] rounded-3xl p-6 md:p-8 flex flex-col shadow-sm hover:shadow-md transition-all">
              <StarRow />
      ...
      <div className="mt-8 bg-[#F5F7FA] border border-[#E2E8F0] rounded-2xl p-5 text-sm text-[#5F6F75] font-sans leading-relaxed">
        {isSpanish ? (
          <>
            <strong className="text-[#14324b]">Nota sobre opiniones verificadas:</strong> Todas las reseñas que se muestran arriba representan comentarios reales de clientes publicados directamente en nuestro Perfil de Empresa de Google. Solo mostramos nombres de clientes verificados y detalles de comentarios reales.
          </>
        ) : (
          <>
            <strong className="text-[#14324b]">Note on Verified Testimonials:</strong> All reviews shown above represent real customer feedback published directly on our Google Business Profile. We display only verified customer names and actual feedback details.
          </>
        )}
      </div>
    </div>
