...
function FAQRow({ question, answer }: FAQItem) {
  ...
  return (
    <div className="border border-[#E6EDF2] rounded-2xl bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      <button
      ...
      >
        ...
        <ChevronDown
          className={`w-5 h-5 text-[#FF8A3D] shrink-0 transition-transform duration-300 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      <div
  ...
  return (
    <section className="py-12 max-w-4xl mx-auto w-full">
      <div className="text-center mb-10">
        <span className="text-[10px] font-bold uppercase text-[#5EC8E5] tracking-widest mb-1 block">
          Have Questions?
        </span>
        <h2 className="font-poppins font-bold text-2xl md:text-3xl text-[#123B5D]">{title}</h2>
        {subtitle && (
